...
df['types'] = df['types'].astype(str)  # Ensure no NaNs or bad types
df['type_encoded'] = types_encoder.fit_transform(df['types'])

# Normalize
scaler = MinMaxScaler()
df['rating_scaled'] = scaler.fit_transform(df[['rating']])
df['user_rating_count_scaled'] = scaler.fit_transform(df[['userRatingCount']])
...
# If category is not venue, set type_encoded to max + 1 (new index for unknown)
unknown_type_index = df['type_encoded'].max() + 1
df['type_encoded'] = df.apply(lambda row: row['type_encoded'] if row['category'] == 'venue' else unknown_type_index, axis=1)
